...
|  | 요구사항 | 이걸 만족시키는 서비스 |
|---|---|---|
| 1 |	수백만 건 이벤트, 확장 가능 | KDS (샤드 확장) |
| 2 | 준실시간 | DS (Firehose는 60초라 애매) |
| 3 | 여러 내부 분석 앱과 공유 | KDS fan-out |
| 4 | NoSQL 저장 전에 PII 마스킹 | 스트림 소비 단계의 Lambda |
| 5 | 리더보드 빠른 조회 | DynamoDB |

### 선지별 풀이 과정

A — DynamoDB 저장 → DynamoDB Streams로 공유

1. "DynamoDB에 쓰기 시 PII를 제거하는 규칙을 설정한다" → 그런 기능 없음
2. DynamoDB Streams는 항목 변경 캡처용이지, 수백만 이벤트를 여러 분석 앱에 뿌리는 범용 스트림 버스가 아님. → 오답

B — Firehose → DynamoDB + S3, Firehose Lambda로 PII 제거, 다른 앱은 S3 소비

1. Firehose는 DynamoDB를 목적지로 지원하지 않음. → 아키텍처 자체가 성립 불가.
2. Firehose 최소 버퍼 60초 → 요구사항 2(준실시간) 약함.
3. 다른 앱이 S3 파일을 읽는 구조 = 배치 소비. 요구사항 3의 "준실시간 공유"와 어긋남. → 오답

C — KDS 스트리밍 → Lambda로 PII 제거 → DynamoDB 저장, 다른 앱은 스트림 소비

1. KDS = 수백만 이벤트 수집 + 샤드로 확장 → 요구사항 1 ✅
2. KDS 지연 ms~초 → 요구사항 2 ✅
3. 다른 분석 앱들이 같은 스트림을 독립 소비(fan-out) → 요구사항 3 ✅
4. Lambda가 스트림에서 읽어 PII 제거 후 DynamoDB에 씀 → 요구사항 4 ✅
5. DynamoDB → 리더보드 저지연 조회 → 요구사항 5 ✅ → 정답

D — 배치 파일을 S3 저장 → Lambda 처리 → S3 갱신 → Lambda가 DynamoDB 저장

1. "배치된 이벤트를 파일로" = 스트리밍이 아님. 요구사항 2 위반.
2. S3 왕복이 두 번 → 엔드투엔드 지연 큼.
3. 다른 앱이 S3 파일 소비 → 준실시간 공유 아님. → 오답
...
대용량 실시간 데이터를 담아두는 분산 스트림 버스
- 샤드 단위로 구성. 샤드 1개당 쓰기 1MB/s 또는 1,000 records/s, 읽기 2MB/s. → 샤드를 늘리면 선형 확장. 수백만 이벤트도 처리 가능.
- 용량 모드 두 가지
  - Provisioned: 샤드 개수를 내가 지정
  - On-demand: AWS가 자동 조절 (운영 부담 ↓)
- 보존 기간: 기본 24시간, 최대 365일까지 연장 가능
- 핵심 특성: 다중 소비자(fan-out)가 데이터를 읽어도 스트림에서 사라지지 않음. 소비자 A, B, C가 같은 이벤트를 각자 독립적으로 읽을 수 있음. 각 소비자는 자기 체크포인트(sequence number)를 따로 관리. → "여러 애플리케이션이 같은 데이터를 공유해야 한다" = KDS의 시그니처 요구사항

1.1. AWS Lambda의 스트림 통합
- KDS를 이벤트 소스 매핑(Event Source Mapping) 으로 붙이면, Lambda가 샤드를 폴링해서 배치 단위로 레코드를 받아 처리함.
- 여기서 PII 마스킹 같은 레코드 변환을 수행하고 결과를 DynamoDB에 쓰면 → "저장되기 전에 마스킹" 요구를 만족.
- Firehose에도 Lambda 변환이 있지만, 그건 목적지 적재 직전 변환.

2. Amazon Kinesis Data Firehose
스트림 데이터를 정해진 목적지 한 곳에 자동 적재해주는 완전관리형 전달 서비스
- 지원 목적지: S3, Redshift, OpenSearch, Splunk, 일부 서드파티 HTTP 엔드포인트 <br>→ DynamoDB는 목적지가 아님.
- 버퍼링: 버퍼 크기(1~128MB) 또는 버퍼 간격(최소 60초) 중 먼저 도달하는 조건에 적재
- 샤드 관리 없음 → 운영은 편하지만 유연성 낮음
- Lambda 변환 지원: 적재 전에 Lambda로 레코드 변환/필터링 가능
- 소비자 구조: 목적지 하나. fan-out 불가.

3. Amazon DynamoDB & DynamoDB Streams
- DynamoDB: 완전관리형 NoSQL. 한 자릿수 밀리초 응답 → 리더보드 조회에 적합.
- DynamoDB Streams: 테이블의 항목 변경 이력(INSERT/MODIFY/REMOVE) 을 시간순으로 캡처.
  - 보존 24시간
  - 중요: 스트림은 이미 테이블에 쓰인 후에 발생하는 이벤트. → "저장 전에 마스킹"에는 절대 쓸 수 없음.
  - 그리고 DynamoDB에는 "쓰기 시 PII를 제거하는 규칙" 같은 기능이 존재하지 않음.

4. PII 마스킹
- 개인 식별 정보를 제거·치환하는 처리.
- AWS에서 흔한 구현: Lambda 변환, Glue DataBrew, Macie(탐지 전용, 마스킹 아님).
- 시험에서는 "저장/공유 전에 처리" 라는 순서가 정답 판별 기준이 되는 경우가 많음.

5. AWS Glue DataBrew
시각적 데이터 준비 도구. 코드 없이 GUI로 데이터를 정제·변환.
- 250개 이상의 사전 정의된 변환 제공: 결측치 처리, 이상치 제거, 포맷 통일, 컬럼 분리/병합
- PII 관련 변환 내장: 마스킹, 치환, 해싱, 암호화, 삭제
- 대상: 데이터 분석가/사이언티스트 (Spark 코드를 못 써도 됨)
- 결과는 레시피(recipe) 로 저장 → 반복 실행 가능
- 소스: S3, Redshift, RDS, Glue Data Catalog

5.1. Glue ETL Job과의 차이
|  | Glue ETL Job | Glue DataBrew |
|---|---|---|
| 방식 |	Spark 코드(Python/Scala) 작성	| GUI 클릭 |
| 대상 | 데이터 엔지니어 |	분석가 |
| 용도 | 프로덕션 파이프라인 | 탐색·정제·전처리 |

6. Amazon Macie
S3에 있는 데이터에서 민감정보를 찾아내는 탐지 서비스. 머신러닝 + 패턴 매칭.

- 하는 일: S3 버킷을 스캔해서 PII·민감 데이터가 어디 있는지 발견하고 알림
  - 이름, 주소, 주민번호/SSN, 신용카드 번호, 여권번호, AWS 자격증명 등
- 버킷 보안 상태도 점검: 공개 설정, 미암호화, 외부 계정 공유 여부
- 결과는 findings로 나오고 EventBridge/Security Hub로 연동
...
여러 상호 연결된 서비스로 구성된 환자 관리 시스템을 운영하고 있습니다.z
